...
const GroupList:FC<{
   ...
   function queryList(){
      client.models.GroupUserRelation.list({filter:{userId:{eq:user.userId}}}).then(res=>{
        ...
        if(arr.length) setCurGroupId(arr[0].data!.id);// init group id.
      });
   }
   useEffect(()=>{
      queryList();
   ...
   return (<ul>{
      group.map((v)=>(<li style={{backgroundColor: "rgb(204, 204, 204)",cursor: "pointer"}} onClick={handleGroupClick.bind(null,v.id)} key={v.id}><span>{v.name}</span></li>))
   }</ul>)
}
